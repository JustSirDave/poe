...
describe('read-only MCP adapter', () => {
  it('negotiates a supported version and exposes only bounded read tools', async () => {
    ...
    const result = await handle({ jsonrpc: '2.0', id: 3, method: 'tools/list' }) as { result: { tools: { name: string; annotations: { readOnlyHint: boolean } }[] } };
    expect(result.result.tools.map(tool => tool.name)).toEqual(['poe_summary', 'poe_findings', 'poe_proposal']);
    expect(result.result.tools.every(tool => tool.annotations.readOnlyHint)).toBe(true);
    expect(await handle({ jsonrpc: '2.0', id: 4, method: 'tools/call', params: { name: 'poe_findings', arguments: { limit: 5000 } } })).toHaveProperty('result.isError', true);
    expect(await handle({ jsonrpc: '2.0', id: 5, method: 'tools/call', params: { name: 'poe_summary', arguments: {} } })).toHaveProperty('result.content');
    expect(await handle({ jsonrpc: '2.0', id: 7, method: 'tools/call', params: { name: 'coach_summary', arguments: {} } })).toHaveProperty('result.content');
